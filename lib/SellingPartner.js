...
class SellingPartner {
  ...
  async _getReport(req_params, report_id) {
    ...
    if (res.processingStatus === 'DONE') {
      ...
      if (this._options.debug_log) {
        console.log('Current status of report ' + req_params.body.reportType + ': ' + res.processingStatus);
      }
  ...
  async _getReportDocument(report_document_id) {
    let res = await this.callAPI({
  ...
  async downloadReport(req_params) {
    ...
    let report_document = await this._getReportDocument(report_document_id);
